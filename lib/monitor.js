...
var config_methods = {
  getIgnoreList: function(){
    ...
    var list = [];
    var fs = require('fs');
    var array = fs.readFileSync(path.join('.', '.watchignore')).toString().split("\n");
    for(i in array) {
      list.push(i);
    }
    return list;
  ...
  getThemeSettings: function(){
    ...
    try {
      var json = fs.readFileSync(config.watch.path+ds+'theme.json');
    } catch (e) {
  ...
  getApiKeyForTheme: function(domain){
    return user_theme_settings.api_keys[config.theme.domain];
  }
}
