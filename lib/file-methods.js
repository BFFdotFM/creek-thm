...
module.exports.util = {
  ...
  checkWhitelist: function(file_path){

    var okay = false;

    for (var i = 0; i < config.watch.whitelist.length; i++) {
      c.debug(config.watch.whitelist[i], 3);
      if(file_path.indexOf(config.watch.whitelist[i]) !== -1){
        c.debug("YEP: "+config.watch.whitelist[i], 3);
        okay = true;
      }
    }
    
    c.debug("Whitelist? " + (okay ? "Yeah." : "No."), 3);
  ...
  prepareFormData: function(type, file_path){
    ...
    var theme = config_methods.getThemeSettings({ 
      fileEdited: file_path, 
      optionsConvertToJSON: true 
    });
    ...
    if(type=="pages"){
      ...
      //Check if Template uses Twig
      if(file_path.indexOf(".twig") !== -1){
        var engine = "twig";
      }
      else{
        var engine = "";
      ...
      var theme_dir = findParentDir.sync(file_path, 'theme.json');

      // console.log(theme_dir);

      var file_with_dir = file_path.replace(theme_dir+"files/", "");
    ...
    return {
      ...
      //Add API key
      key: config_methods.getApiKeyForTheme(file_path),
  ...
  getUrl: function(type, file_path){
    cfg = config_methods.getThemeSettings({fileEdited: file_path});
    var ext = path.extname(file_path);
    var name = path.basename(file_path, ext);
    return protocol + cfg.domain
           + "/api/v2/themes/" + cfg.id
           + "/" + type + "/" + name;
  }
}
...
module.exports.main = {
  ...
  changed: function(file_path){
    ...
    if(methods.checkWhitelist(file_path) == false){
      return false;
    }
    
    //Get the theme settings for the file provided
    config_methods.getThemeSettings({fileEdited: file_path});

    //!!! theme.json is always edited anyway
    //If it's theme.json, then update the internal theme config
    // if(file_path.indexOf(ds+"theme.json")){
    //   config_methods.getThemeSettings(file_path);
    // }
